...
class _ClippedBottomBarState extends State<ClippedBottomBar> with TickerProviderStateMixin {
  ...
  void didChangeDependencies() {
    ...
    _animationIconCircles[widget.index] =
        Tween<double>(begin: -1.5, end: 1.0).animate(_animationControllers[widget.index]);
      ...
      child: SizedBox(
        ...
        child: Stack(
          children: [
                      ...
                      builder: (_, child) {
                        return Transform(
                          transform: Matrix4.translationValues(
                              0.0, -30.0 * (_animations[index] != null ? _animations[index].value : 0.0), 0.0),
                          child: child,
                        );
                      },
            ...
            Transform(
              transform: Matrix4.translationValues(
                  (_animationClipper != null ? _animationClipper.value : size.width / 2 - 35) + 10, -30, 0.0),
              child: Stack(
                ...
                return AnimatedBuilder(
                  ...
                  builder: (_, child) {
                    return Transform(
                      transform: Matrix4.translationValues(
                          0.0,
                          -17.5 * (_animationIconCircles[index] != null ? _animationIconCircles[index].value : -1.5),
                          0.0),
                      child: child,
                    );
                  },
                    ...
                    return GestureDetector(
                      onTap: () {
                        if (!_animationControllers[index].isAnimating) {
                          widget.onTap(index);
                          _animations[index] =
                              Tween<double>(begin: 0.0, end: 1.0).animate(_animationControllers[index]);
                          _animationIconCircles[index] =
                              Tween<double>(begin: -1.5, end: 1.0).animate(_animationControllers[index]);
                          _animationClipper = Tween<double>(
                                  begin: _getBottomClipper(context, _previousIndex),
                                  end: _getBottomClipper(context, index))
...
class BottomBarClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    Path path = Path();
    path.arcTo(Rect.fromLTWH(_animationClipper != null ? _animationClipper.value : size.width / 2 - 35, -30, 70, 60),
        pi, -pi, true);
